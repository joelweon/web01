/* 주제: HttpServletRequest 사용 - 페이지 이동 기법(Refresh) III
...
//    Redirect 응답하기
//    응답헤더에 상태 값을 xxx 로 응답한다.
//    => 출력 버퍼에 저장된 내용을 모두 버린다.
//      - 출력 버퍼의 기본 크기는? 서블릿 컨테이너마다 다르다. 톰캣 서버-> 8KB(8192B)이다.
//      - 만약에 sendRedirect()를 호출하기 전에 버퍼 크기를 넘게 출력했다면?
//        => 버퍼가 꽉차면 자동으로 클라이언트에게 내보낸다.
//        => sendRedirect()를 할 수 없다. 호출하면 오류가 발생한다.
 ...
 * 클라이언트에게 출력하지 않고 바로 리다이렉트한다.
 * 내용이 필요없다.
 * 
 * 페이지 이동은 redirect/ refresh 두 가지만 있다.
 * 무언가를 출력하고 싶다면 refresh 아니면 redirect. 
 * */
